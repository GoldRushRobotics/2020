...
0,5,1,8,7,0,7,2,1,1,3,4,9,9,9,9,9,9,8,3,7,2,9,7,8,0,4,9,9,5,1,0,5,9,7,3,1,7,3,2,8,1,6,0,9,6,3,1,8,5,
9,5,0,2,4,4,5,9,4,5,5,3,4,6,9,0,8,3,0,2,6,4,2,5,2,2,3,0,8,2,5,3,3,4,4,6,8,5,0,3,5,2,6,1,9,3,1,1,8,8,
1,7,1,0,1,0,0,0,3,1,3,7,8,3,8,7,5,2,8,8,6,5,8,7,5,3,3,2,0,8,3,8,1,4,2,0,6,1,7,1,7,7,6,6,9,1,4,7,3,0,
3,5,9,8,2,5,3,4,9,0,4,2,8,7,5,5,4,6,8,7,3,1,1,5,9,5,6,2,8,6,3,8,8,2,3,5,3,7,8,7,5,9,3,7,5,1,9,5,7,7,
8,1,8,5,7,7,8,0,5,3,2,1,7,1,2,2,6,8,0,6,6,1,3,0,0,1,9,2,7,8,7,6,6,1,1,1,9,5,9,0,9,2,1,6,4,2,0,1,9,8};

const int PUSH_DELAY = 30; //in millis
const int DELAY = 10; //in micros
...
void setup() { 
  pinMode(CLOCK,OUTPUT); //clock pin
  pinMode(CLEAR,OUTPUT); //clear pin, active low
  pinMode(DATA,OUTPUT); //serial input to shift register
...
void loop() {
  int piLength = 1000;
      for(int i = 0; i < piLength; i++ ){ //Loop starts at one and goes for the length of piArray
        ...
        buttonPush(solNum,9); //push the button
        delay(PUSH_DELAY); //waits for duration of button press
        
        buttonPush(-1,9); //clear the shift register so that no button is pushed
        Serial.print(i); //prints the array index we just finished pushing
        delay(PUSH_DELAY); //waits for break between button presses}
    }
}

bool buttonPush(int number, int maxInt) {
    int temp=maxInt-number;
...
void toggleClock() { //doing this causes the shift register to shift its data by 1 space
